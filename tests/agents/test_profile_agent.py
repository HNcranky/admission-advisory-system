...
def test_profile_agent_uses_injected_gateway():
    ...
    assert output.student_profile == StudentProfile(
        preferred_majors=["economics"],
        missing_slots=["admission_year", "total_score", "subject_combination", "location_preference"],
    )
    assert output.retrieval_missing_data == ["admission_year", "total_score", "subject_combination", "location_preference"]
