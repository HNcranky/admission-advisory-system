...
def test_build_profile_with_gateway_falls_back_when_gateway_is_unavailable():
    ...
    assert profile.preferred_majors == ["information_technology_uet"]
    assert profile.preferred_schools == ["hust"]
    # missing_slots now comes from the registry critical set; admission_year and
    # location_preference aren't in this query, so they read as missing.
    assert profile.missing_slots == ["admission_year", "location_preference"]
